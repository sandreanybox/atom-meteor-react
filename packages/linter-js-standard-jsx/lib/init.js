...
module.exports = {
  config: {
    style: {
      type: 'string',
      default: 'standard-jsx',
      enum: ['standard-jsx', 'standard', 'semi-standard', 'happiness', 'uber-standard']
    },
  ...
  activate: function () {
    ...
    .install('linter-js-standard-jsx')
    .then(function () {
      var config = atom.config.get('linter-js-standard-jsx')
      ...
      self.subscriptions.add(atom.config.observe('linter-js-standard-jsx', function (config) {
        // Cache config
        self.cache.set('config', config)
      }))
    })
